...
object Tools {
  ...
  fun setSystemBarColorDialog(context: Context, dialog: Dialog, @ColorRes i: Int) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      if (window != null) {
        window.addFlags(Integer.MIN_VALUE)
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
        window.statusBarColor = ContextCompat.getColor(context, i)
      }
    }
  }
  ...
  fun displayImageRound(context: Context, imageView: ImageView, @DrawableRes i: Int) {
    val options = RequestOptions().centerCrop().circleCrop()
    Glide.with(context).load(i).apply(options).into(imageView)
  }
  ...
  fun getFormattedDateShort(l: Long): String {
    return SimpleDateFormat("MMM dd, yyyy", Locale.getDefault()).format(Date(l))
  }

  fun getFormattedDateSimple(l: Long): String {
    return SimpleDateFormat("MMMM dd, yyyy", Locale.getDefault()).format(Date(l))
  }
  ...
  fun getFormattedTimeEvent(l: Long): String {
    return SimpleDateFormat("h:mm a", Locale.getDefault()).format(Date(l))
  }

  fun getEmailFromName(str: String?): String? {
    return if (str == null || str == "") str
    else str.replace(" ".toRegex(), ".").toLowerCase(Locale.getDefault()) + "@mail.com"
  }
  ...
  fun changeNavigationIconColor(toolbar: Toolbar, @ColorInt i: Int) {
  ...
  fun toCamelCase(str: String): String {
    val strLower = str.toLowerCase(Locale.getDefault())
    val stringBuilder = StringBuilder()
    var obj: Any? = 1
    for (c in strLower.toCharArray()) {
